...
void LoadData(CityMap& cityLookup) {
  ifstream input(kPlaceDataFile.c_str());
  string data, name;
  ...
  while (getline(input, name)) {
    // The coordinates are put into a stringstream so that we can extract them
    // as doubles.
    getline(input, data);
    stringstream converter;
    converter << data;
    ...
    converter >> location.latitude;
    converter >> location.longitude;
    ...
    //
    // Both are very similar in terms of efficiency, but the [] notation doesn't
    // work for multimaps as we'll see later.
...
void FindNearbyCities(CityMap& cityLookup, CoordMap& latLookup, CoordMap& longLookup, double radius, CityLocation center) {
  ...
  CoordMap::iterator latBegin = latLookup.lower_bound(center.latitude - radius);
  CoordMap::iterator latEnd = latLookup.upper_bound(center.latitude + radius);
  for (CoordMap::iterator itr = latBegin; itr != latEnd; ++itr)
    nearbyLats.insert(itr->second);
  
  // We repeat the above approach, but with candidate cities according to
  // longitude.
  set<string> nearbyLongs;
  CoordMap::iterator longBegin = longLookup.lower_bound(center.longitude - radius);
  CoordMap::iterator longEnd = longLookup.upper_bound(center.longitude + radius);
  for (CoordMap::iterator itr = longBegin; itr != longEnd; ++itr)
  ...
  for (size_t i = 0; i < candidates.size(); ++i) {
    CityLocation city = cityLookup[candidates[i]];
...
int main() {
  ...
  while (true) {
    ...
    CityMap::iterator cityItr = cityLookup.find(name);
